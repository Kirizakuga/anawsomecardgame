...
func _can_drop_data(_at_position: Vector2, data: Variant) -> bool:
	if is_occupied():
		return false
	if data is Dictionary and data.get("type") == "card":
		var card_data: CardResource = data.get("card_data")
		return card_data is CreatureResource
	return false

func _drop_data(_at_position: Vector2, data: Variant) -> void:
	var source_view: CardView = data.get("source_view")
	var card_data: CardResource = data.get("card_data")
	if source_view and source_view.get_parent():
