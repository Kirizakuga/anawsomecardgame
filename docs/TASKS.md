...
- Spec: Persist a built deck (Hero + card list + landscape list) to disk and reload it. Format decision (Resource vs JSON) should follow whatever is resolved in TDD.md §6 open question — flag if unresolved.
...
- Spec: Attacking a Pact ally in the same turn as breaking the Pact triggers the Betrayal bonus per `GDD.md` §6.
...
- Spec: Player in last place (lowest Life) receives bonus Essence per turn per `GDD.md` §6.
...
*(Empty at time of writing — this section exists so any contributor, human or AI, has a designated place to flag ambiguity in GDD.md/TDD.md instead of guessing. Format: `[Task ID] — [Question] — raised by [agent/person] on [date]`)*
